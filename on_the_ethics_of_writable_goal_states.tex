...
The HGI pipeline itself is clean: hierarchical detection $\to$ plan segmentation (bootstrapped from accessibility tree + LLM labels) $\to$ goal-conditioned preference model $\to$ actor with scratchpad conditioning $\to$ distill-search-distill loop. The recursive ``scale and pad'' trick for both spatial and temporal hierarchies is elegant---treating plan detection as analogous to object detection across time is a natural move.
...
\begin{itemize}
  ...
  \item A \textbf{goal-conditioned preference model} scores candidate actions given history and the current goal.
